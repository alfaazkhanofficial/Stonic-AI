...
from stonic.agent.loop import AgentLoop
...
class CoreService:
    def __init__(self, db: Database, config: Configuration, diagnostics: Diagnostics,
                 events: EventBus, provider: IntelligenceProvider, credentials=None) -> None:
        ...
        self.agent_memory = Memory(self.agent_store)
        self.scheduler.memory = self.agent_memory
        self.agent_router = ModelRouter(provider, credentials)
        self.agent_skills = SkillLibrary(self.agent_store)
        self.agent_loop = AgentLoop(self.tools, self.agent_router, self.agent_store, self.agent_memory, events, self.agent_skills)
        self.system_control = SystemControl(self.agent_store, diagnostics.data_dir)
        self.system_control.register(self.tools)
        self.agent_sessions: dict[str, str] = {}   # session_id -> most recent agent goal id
    ...
    async def chat(self, request: ChatInput, on_token=None) -> dict:
        async with self.lock:
            ...
            try:
                if request.content.strip() == "/status":
                ...
                elif (not self.config.values.llm_model or (hasattr(self.provider, "available") and not self.provider.available(self.config.values))) and not (self.config.values.agent_enabled and self.agent_router.usable(self.config.values)):
                    ...
                    elif settings.agent_enabled and self.agent_router.usable(settings):
    ...
    @staticmethod
    def describe_confirmation(confirm: list[dict]) -> str:
        def one(item: dict) -> str:
            args = ", ".join(f"{k}={v}" for k, v in item["arguments"].items() if isinstance(v, (str, int, float, bool)))
            return f"{item['tool']}({args})" if args else item["tool"]
        return "STONIC wants to: " + "; ".join(one(c) for c in confirm) + \
               ". Review the exact targets and approve or decline in Task activity."

    async def run_agent(self, request: ChatInput) -> str:
        """Route a request through the V3 goal loop. Falls back to the old plan/answer path only when the agent is
        disabled or no reasoning provider is configured for it (see ``CoreService.chat``)."""
        active_id = self.agent_sessions.get(request.session_id)
        if active_id:
            row = self.agent_store.load_goal(active_id)
            if row and row["status"] == "waiting_approval":
                resumed = self.agent_loop.resume(active_id)
                confirm = self.agent_loop.describe_pending(resumed, self.config.values) if resumed else []
                return "Your previous request is still waiting for approval. " + self.describe_confirmation(confirm)
        self.state.transition(Activity.EXECUTING)
        goal = self.agent_loop.start(request.session_id, request.content, self.config.values)
        result = await self.agent_loop.run(goal, self.config.values)
        self.agent_sessions[request.session_id] = result["goal_id"]
        self.events.emit("panel", {"panel": "activity"})
        if result["status"] == "waiting_approval":
            return self.describe_confirmation(result["confirm"])
        if result["status"] in {"failed", "stopped"}:
            self.events.publish("agent", f"Goal {result['status']}: {result['message'][:200]}",
