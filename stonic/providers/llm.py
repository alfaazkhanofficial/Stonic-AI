...
_KEEP_SCHEMA_KEYS = {"type", "description", "enum", "properties", "required", "items", "minimum", "maximum"}


def wire_name(name: str) -> str:
    """OpenAI function names allow letters, digits, '_' and '-': 'files.read' travels as 'files__read'."""
    return name.replace(".", "__")


def tool_name(wire: str) -> str:
    return wire.replace("__", ".")


def simplify_schema(schema: dict) -> dict:
    """Pydantic JSON Schema -> the small subset every OpenAI-compatible endpoint (including Gemini's) accepts."""
    ...
    def walk(node, depth=0):
        ...
        if not isinstance(node, dict) or depth > 12:
            return node
        if "$ref" in node:
            return walk(defs.get(str(node["$ref"]).rsplit("/", 1)[-1], {}), depth + 1)
        for union in ("anyOf", "oneOf"):
            if union in node:
                options = [o for o in node[union] if not (isinstance(o, dict) and o.get("type") == "null")]
                rest = {k: v for k, v in node.items() if k != union}
                node = {**walk(options[0], depth + 1), **rest} if options else rest
                break
        out = {}
        for key, value in node.items():
            if key not in _KEEP_SCHEMA_KEYS:
                continue
            if key == "properties":
                out[key] = {name: walk(prop, depth + 1) for name, prop in value.items()}
            elif key == "items":
...
class OpenAICompatibleProvider:
    ...
    def __init__(self, secrets=None) -> None:
        ...
        self.resolvers: dict[str, Callable[[], str]] = {}   # base_url -> key lookup (agent profiles)
    ...
    def provider_name(settings: Settings) -> str:
        ...
        return "xKiro" if host == XKIRO_HOST else "Gemini" if host == GEMINI_HOST else "The configured provider"
    ...
    def _credential(self, settings: Settings | None) -> str:
        if settings is None:
            return ""
        resolver = self.resolvers.get(settings.llm_base_url)
    ...
    def is_gemini(settings: Settings) -> bool:
        return urlsplit(settings.llm_base_url).hostname == GEMINI_HOST

    async def act(self, settings: Settings, messages: list[dict], tools: list[dict], *, max_tokens: int = 2048,
                  reasoning: str | None = None, tool_choice: str = "auto") -> dict:
        """One agent turn with native tool calling.

        Returns ``{content, tool_calls[{id,name,arguments,raw}], assistant_message, usage, finish_reason}``. ``arguments`` is
        None when the model sent unparseable JSON (the loop reports that back instead of guessing). The assistant message is
        rebuilt from the provider's own tool_call objects so provider-specific fields (Gemini thought signatures) survive
        the round trip.
        """
        specs = [{"type": "function", "function": {"name": wire_name(t["name"]), "description": str(t["description"])[:500],
                  "parameters": simplify_schema(t["arguments"])}} for t in tools]
        body = {"model": settings.llm_model, "messages": messages, "stream": False, "max_tokens": max_tokens,
                **self._latency_fields(settings, planning=True)}
        if not self.is_gemini(settings):
            body["temperature"] = 0.2          # Gemini deprecated sampling parameters; only send them elsewhere
        if specs:
            body["tools"], body["tool_choice"] = specs, tool_choice
        if reasoning and self.is_gemini(settings):
            body["reasoning_effort"] = reasoning
        try:
            response = await self._post_json(settings, body)
        except ProviderError as error:
            if error.status_code in {400, 422} and "reasoning_effort" in body and "reasoning" in str(error).casefold():
        ...
        try:
            ...
            raise ProviderError("The provider returned an invalid agent response.", retryable=True) from error
        ...
        for index, raw in enumerate(message.get("tool_calls") or []):
            ...
            call_id = raw.get("id") or f"call_{index}_{random.randrange(10**8):08d}"
            text = function.get("arguments") or "{}"
            try:
                arguments = json.loads(text) if isinstance(text, str) else dict(text)
                if not isinstance(arguments, dict):
                    arguments = None
            except (ValueError, TypeError):
                arguments = None
            calls.append({"id": call_id, "name": tool_name(str(function.get("name", ""))), "arguments": arguments})
            raw_calls.append({**raw, "id": call_id, "type": "function", "function": {**function, "arguments": text if isinstance(text, str) else json.dumps(text)}})
        ...
        return {"content": content.strip(), "tool_calls": calls, "assistant_message": assistant, "finish_reason": choice.get("finish_reason"),
