...
class VoiceService:
    ...
    async def run_request(self, request: str) -> dict:
        ...
        waiting = core.db.query("SELECT COUNT(*) AS n FROM jobs WHERE session_id=? AND status='waiting_approval'",
                                (self.session_id,))[0]["n"]
        agent_goal_id = core.agent_sessions.get(self.session_id)
        agent_waiting = bool(agent_goal_id and (core.agent_store.load_goal(agent_goal_id) or {}).get("status") == "waiting_approval")
        return {"ok": True, "reply": str(message.get("content", "")), "approval_required": bool(waiting) or agent_waiting}

    async def run_confirmation(self, approved: bool) -> dict:
        """The ``confirm_pending_action`` tool: apply a spoken yes/no to the session's waiting agent goal. Gated by
        ``voice_can_confirm`` at the tool-declaration level (see ``live.build_declarations``); a system-critical action
        still needs the screen, since ``AgentLoop`` marks those risk items even when the caller passes approve_all."""
        core = self.core
        goal_id = core.agent_sessions.get(self.session_id)
        row = core.agent_store.load_goal(goal_id) if goal_id else None
        if not row or row["status"] != "waiting_approval":
            return {"ok": False, "error": "Nothing is waiting for confirmation."}
        resumed = core.agent_loop.resume(goal_id)
        if resumed is None:
            return {"ok": False, "error": "That confirmation has expired."}
        pending = core.agent_loop.describe_pending(resumed, core.config.values)
        if any(item["risk"] == "system-critical" for item in pending):
            return {"ok": False, "error": "This touches a system-critical location and needs confirmation on screen, not by voice."}
        if not approved:
            result = core.agent_loop.deny(goal_id)
        else:
            result = await core.agent_loop.run(resumed, core.config.values, approve_all=True)
        core.append(self.session_id, "assistant", result["message"])
        core.events.emit("conversation", {"session_id": self.session_id})
        core.events.emit("panel", {"panel": "activity"})
        return {"ok": True, "reply": result["message"], "approval_required": result["status"] == "waiting_approval"}
