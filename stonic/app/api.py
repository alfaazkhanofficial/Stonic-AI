...
def create_app(data_dir: Path | None = None, token: str | None = None) -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        ...
        try:
            ...
            core = CoreService(db, config, diagnostics, events, provider, credentials)
            app.state.core = core
    ...
    async def agent_skills_list(status: str | None = None):
        return core().agent_skills.list(status)

    @app.post("/api/agent/skills/propose")
    async def agent_skills_propose(body: dict):
        goal_id, name = body.get("goal_id"), body.get("name")
        if not isinstance(goal_id, str) or not isinstance(name, str):
            raise HTTPException(422, "goal_id and name are required")
        row = core().agent_store.load_goal(goal_id)
        if not row:
            raise HTTPException(404, "Goal does not exist")
        try:
            return core().agent_skills.propose(name, row["goal"], row["payload"].get("steps", []))
        except ValueError as error:
            raise HTTPException(422, str(error))

    @app.post("/api/agent/skills/{identifier}/decision")
    async def agent_skills_decision(identifier: str, body: dict):
        if type(body.get("approved")) is not bool:
            raise HTTPException(422, "A boolean 'approved' decision is required")
        if not core().agent_skills.decide(identifier, body["approved"]):
            raise HTTPException(404, "Skill is not pending")
        return {"id": identifier, "approved": body["approved"]}

    @app.delete("/api/agent/skills/{identifier}")
    async def agent_skills_forget(identifier: str):
        if not core().agent_skills.forget(identifier):
            raise HTTPException(404, "Skill does not exist")
        return {"deleted": identifier}

    @app.post("/api/agent/stop")
    async def agent_stop():
        """Kill switch: stop the active goal now, and mark any other running/waiting goal as stopped so nothing
        queued resumes silently. The active generation is cancelled through the same path as /api/chat/cancel."""
        stopped = []
        for row in core().agent_store.list_goals(statuses=("running", "waiting_approval")):
            core().agent_store.save_goal(row["id"], None, row["session_id"], row["goal"], "stopped", row["autonomy"],
                                         {**core().agent_store.load_goal(row["id"])["payload"], "pending_calls": []})
            stopped.append(row["id"])
        cancelled = core().cancel()
        return {"cancelled_active_request": cancelled, "stopped_goals": stopped}
    ...
    async def agent_goals(session_id: str | None = None):
        return core().agent_store.list_goals(session_id)

    @app.get("/api/agent/goals/{identifier}")
    async def agent_goal(identifier: str):
        detail = core().agent_loop.explain(identifier)
        if detail["goal"] is None:
            raise HTTPException(404, "Goal does not exist")
        if detail["status"] == "waiting_approval":
            resumed = core().agent_loop.resume(identifier)
            detail["confirm"] = core().agent_loop.describe_pending(resumed, core().config.values) if resumed else []
        return detail

    @app.post("/api/agent/goals/{identifier}/decision")
    async def agent_goal_decision(identifier: str, body: dict):
        if type(body.get("approved")) is not bool:
            raise HTTPException(422, "A boolean 'approved' decision is required")
        if core().lock.locked():
        ...
        async with core().lock:
            ...
            try:
                if not body["approved"]:
                    result = core().agent_loop.deny(identifier)
                    if result is None:
                        raise HTTPException(404, "Goal is not waiting for approval")
                else:
                    resumed = core().agent_loop.resume(identifier)
                    if resumed is None:
                        raise HTTPException(404, "Goal is not waiting for approval")
                ...
                if result["status"] != "waiting_approval":
                    core().append(resumed.session_id if body["approved"] else result["goal_id"], "assistant", result["message"])
                    core().events.emit("conversation", {"session_id": resumed.session_id if body["approved"] else result["goal_id"]})
                core().events.emit("panel", {"panel": "activity"})
                return result
            finally:
                core().active = None
                core().state.transition(Activity.IDLE)

    @app.post("/api/agent/goals/{identifier}/resume")
    async def agent_goal_resume(identifier: str):
        if core().lock.locked():
            raise HTTPException(409, "Another request is active")
        resumed = core().agent_loop.resume(identifier)
        if resumed is None:
            raise HTTPException(404, "Goal is not resumable")
        async with core().lock:
            core().active = asyncio.current_task()
            core().state.transition(Activity.EXECUTING)
            try:
                return await core().agent_loop.run(resumed, core().config.values)
            finally:
                core().active = None
                core().state.transition(Activity.IDLE)

    @app.get("/api/agent/lessons")
    async def agent_lessons(status: str | None = None):
        return core().agent_memory.lessons(status)

    @app.post("/api/agent/lessons/{identifier}/decision")
    async def agent_lesson_decision(identifier: str, body: dict):
        if type(body.get("approved")) is not bool:
            raise HTTPException(422, "A boolean 'approved' decision is required")
        if not core().agent_memory.decide_lesson(identifier, body["approved"]):
            raise HTTPException(404, "Lesson is not pending")
        return {"id": identifier, "approved": body["approved"]}

    @app.delete("/api/agent/lessons/{identifier}")
    async def agent_lesson_forget(identifier: str):
        if not core().agent_memory.forget_lesson(identifier):
            raise HTTPException(404, "Lesson does not exist")
        return {"deleted": identifier}
