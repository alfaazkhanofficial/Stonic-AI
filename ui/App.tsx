  ...
  return <PanelLayoutContext.Provider value={{pinned:pinned.map(p=>PANEL_TITLES[p]),pin:pinPanel}}><div className={`app ${reduced ? 'reduced-motion' : ''} density-${config?.values.interface_density || 'comfortable'}`}>
    ...
    <main className="dashboard">
      ...
      <section className="center-workspace">
        ...
        <footer className="workspace-footer"><div className="capability-actions"><button onClick={() => void captureScreen()}><Monitor size={13}/><span>SCREEN</span></button><button onClick={() => setPanel('permissions')}><ShieldCheck size={13}/><span>PC CONTROL</span></button></div><span className="workspace-version">STONIC</span></footer>
      </section>
