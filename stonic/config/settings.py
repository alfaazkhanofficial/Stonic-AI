...
class Settings(Contract):
    ...
    agent_enabled: bool = Field(default=True, title="Agent mode", json_schema_extra=metadata("Agent",
        "Let STONIC pursue multi-step goals on its own: plan, act, verify and recover. Off keeps the older single-plan behaviour."))
    agent_fast_consent: bool = Field(default=False, title="Allow Google Gemini for reasoning", json_schema_extra=metadata("Privacy",
        "Send agent requests, tool results and relevant memories to Google's Gemini API for fast reasoning. Uses the Gemini key saved under Voice. Off keeps everything on your configured provider."))
    agent_base_url: str = Field(default="https://generativelanguage.googleapis.com/v1beta/openai", max_length=500, title="Fast reasoning endpoint", json_schema_extra=metadata("Agent",
        "OpenAI-compatible endpoint for the fast and strong agent models. Defaults to Gemini's OpenAI-compatible API."))
    agent_fast_model: str = Field(default="gemini-3.5-flash-lite", max_length=120, title="Fast model (routine steps)", json_schema_extra=metadata("Agent",
        "Low-latency model for ordinary agent steps."))
    agent_strong_model: str = Field(default="gemini-3.6-flash", max_length=120, title="Strong model (planning and recovery)", json_schema_extra=metadata("Agent",
        "Used for planning, recovery after failures and summaries."))
    agent_autonomy: Literal["ask_every_step", "ask_on_risk", "autonomous"] = Field(default="ask_on_risk", title="Autonomy", json_schema_extra=metadata("Agent",
        "Ask before every action, ask before deletions/uninstalls/system changes (default), or act on its own inside safe limits. Critical system actions always ask."))
    agent_max_steps: int = Field(default=24, ge=1, le=100, title="Steps per goal", json_schema_extra=metadata("Agent",
        "Stop a goal cleanly, with a report, after this many model turns."))
    agent_max_seconds: int = Field(default=300, ge=10, le=3600, title="Seconds per goal", json_schema_extra=metadata("Agent",
        "Stop a goal cleanly after this much running time (time spent waiting for your confirmation is not counted)."))
    agent_max_tokens: int = Field(default=120000, ge=5000, le=1000000, title="Tokens per goal", json_schema_extra=metadata("Agent",
        "Stop a goal cleanly after roughly this many model tokens."))
    agent_auto_approve: list[str] = Field(default_factory=list, max_length=60, json_schema_extra={"internal": True})
    agent_protected_paths: list[str] = Field(default_factory=list, max_length=60, json_schema_extra={"internal": True})
    voice_can_confirm: bool = Field(default=False, title="Allow confirming actions by voice", json_schema_extra=metadata("Voice",
        "Let a spoken 'yes' confirm deletions and uninstalls, only while push-to-talk is held. Critical system actions always need the screen."))

    @field_validator("agent_fast_model", "agent_strong_model")
    @classmethod
    def validate_agent_model(cls, value: str) -> str:
        import re
        value = value.strip()
        if not re.fullmatch(r"[A-Za-z0-9._:/-]{3,120}", value):
            raise ValueError("Use the model identifier only, for example gemini-3.5-flash-lite")
        return value

    @field_validator("agent_base_url")
    @classmethod
    def validate_agent_endpoint(cls, value: str) -> str:
        return cls.validate_endpoint(value)
