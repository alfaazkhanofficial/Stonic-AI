...
CONFIRM_TOOL_NAME = "confirm_pending_action"
INITIAL_BACKOFF = 3.0
MAX_BACKOFF = 60.0
QUOTA_BACKOFF = 45.0
...
def build_declarations(with_behavior: bool, confirm_enabled: bool = False) -> list[dict]:
    fn: dict[str, Any] = {
        "name": TOOL_NAME,
        "description": (
            ...
            "screen capture, system or gaming status, and every action. If the reply says an action needs "
            "confirmation, read the exact targets aloud and ask the user to say yes or no; do not assume."
            " Returns {ok, reply, approval_required}."),
        "parameters": {
            "type": "OBJECT",
            "properties": {"request": {
                "type": "STRING",
    ...
    if confirm_enabled:
        declarations.append({
            ...
            "description": ("Call this ONLY after you have read the pending action's exact targets aloud and the "
                            "user has clearly said yes or no to confirm or cancel it. Never call it on a guess."),
            "parameters": {"type": "OBJECT", "properties": {"approved": {"type": "BOOLEAN",
                          "description": "true if the user said yes/confirm, false if they said no/cancel."}},
                          "required": ["approved"]},
        })
    if with_behavior:
        for entry in declarations:
            entry["behavior"] = "BLOCKING"   # the model waits for the answer; explicit because newer models default to async
...
def build_config(settings, system_prompt: str, handle: str | None, tier: int) -> dict:
    ...
    config: dict[str, Any] = {
        ...
        "tools": build_declarations(with_behavior=tier < 2, confirm_enabled=getattr(settings, "voice_can_confirm", False)),
...
class LiveRunner:
    ...
    async def _run_tool(self, session, call_id: str, name: str, args: dict) -> None:
        try:
            ...
            elif name == CONFIRM_TOOL_NAME:
                result = await self.host.run_confirmation(bool(args.get("approved", False)))
            else:
